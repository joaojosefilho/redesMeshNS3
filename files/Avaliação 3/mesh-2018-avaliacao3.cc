...
/*Program Arguments:
    --x-size:                Number of nodes in a row grid. [3] [2]
    --y-size:                Number of rows in a grid. [3] [1]
    --numFlows:              Number of flows. [1] [1]
    --seed:                  Seed [1] [1]
    --standardPhy:           PHY layer Definition. WIFI_PHY_STANDARD_80211n_5GHZ by default [1]
    --step:                  Size of edge in our grid, meters. [100 m] [101]
    --start:                 Maximum random start delay, seconds. [0.1 s] [0.1]
    --time:                  Simulation time, seconds [100 s] [50]
    --packet-interval:       Interval between packets in UDP ping, seconds [0.001 s] [0.1]
    --timeStartFlowSources:  Time to start source flows, seconds [1 s] [1]
    --packet-size:           Size of packets in UDP ping [1024]
    --interfaces:            Number of radio interfaces used by each mesh point. [1] [1]
    --channels:              Use different frequency channels for different interfaces. [0] [true]
    --pcap:                  Enable PCAP traces on interfaces. [0] [false]
    --stack:                 Type of protocol stack. ns3::Dot11sStack by default [ns3::Dot11sStack]
    --root:                  Mac address of root mesh point in HWMP [ff:ff:ff:ff:ff:ff]
...
{
  CommandLine cmd;
  cmd.AddValue ("n", "Usado como referência para escrever no csv", m_n);
...
MeshAulaRPG::Run ()
{
  ...
  //out.open("resultadosAvaliacao3.txt", std::ios::app);  
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
  {
        ...
        if(m_packetInterval == 0.1){
            ref01 = true;                
            
            out.open("avaliacao3/01/fluxo.csv", std::ios::app);
            out<< t.sourceAddress << " -> " << t.destinationAddress << ";";            
            out.close();
            
            out.open("avaliacao3/01/taxaEntrega.csv", std::ios::app);
            out<<(i->second.rxPackets * 100.0)/ (i->second.txPackets)  << ";";
            out.close();
            
            out.open("avaliacao3/01/vazao.csv", std::ios::app);
            out<<i->second.rxBytes * 8.0 / 10.0 / 1024 / 1024  << ";";
            out.close();
            
            out.open("avaliacao3/01/atrasoMedio.csv", std::ios::app);
            out<<(i->second.delaySum.GetSeconds()) / (i->second.rxPackets) << ";";
            out.close();
            
            out.open("avaliacao3/01/variacaoAtraso.csv", std::ios::app);
            out<<(i->second.jitterSum.GetSeconds()) / (i->second.rxPackets -1)<< ";";
            out.close();
            
                
        }else if(m_packetInterval == 0.01){
            ref001 = true;
            
            out.open("avaliacao3/001/fluxo.csv", std::ios::app);
            out<< t.sourceAddress << " -> " << t.destinationAddress << ";";            
            out.close();
            
            out.open("avaliacao3/001/taxaEntrega.csv", std::ios::app);
            out<<(i->second.rxPackets * 100.0)/ (i->second.txPackets)  << ";";
            out.close();
            
            out.open("avaliacao3/001/vazao.csv", std::ios::app);
            out<<i->second.rxBytes * 8.0 / 10.0 / 1024 / 1024  << ";";
            out.close();
            
            out.open("avaliacao3/001/atrasoMedio.csv", std::ios::app);
            out<<(i->second.delaySum.GetSeconds()) / (i->second.rxPackets) << ";";
            out.close();
            
            out.open("avaliacao3/001/variacaoAtraso.csv", std::ios::app);
            out<<(i->second.jitterSum.GetSeconds()) / (i->second.rxPackets -1)<< ";";
            out.close();
                                
                
        }else if(m_packetInterval == 0.001){
            ref0001 = true;
                
            out.open("avaliacao3/0001/fluxo.csv", std::ios::app);
            out<< t.sourceAddress << " -> " << t.destinationAddress << ";";            
            out.close();
            
            out.open("avaliacao3/0001/taxaEntrega.csv", std::ios::app);
            out<<(i->second.rxPackets * 100.0)/ (i->second.txPackets)  << ";";
            out.close();
            
            out.open("avaliacao3/0001/vazao.csv", std::ios::app);
            out<<i->second.rxBytes * 8.0 / 10.0 / 1024 / 1024  << ";";
            out.close();
            
            out.open("avaliacao3/0001/atrasoMedio.csv", std::ios::app);
            out<<(i->second.delaySum.GetSeconds()) / (i->second.rxPackets) << ";";
            out.close();
            
            out.open("avaliacao3/0001/variacaoAtraso.csv", std::ios::app);                    
            out<<(i->second.jitterSum.GetSeconds()) / (i->second.rxPackets -1)<< ";";
            out.close();
        ...
        if (i->second.rxBytes > 0){
            ...
            //out<<"  Throughput:                  " << i->second.rxBytes * 8.0 / 10.0 / 1024 / 1024  << " Mbps\n";
            //std::cout << "  TxBytes:                     " << i->second.txBytes << "\n";
            //out<<"  TxBytes:                     " << i->second.txBytes << "\n";            
            //std::cout << "  RxBytes:                     " << i->second.rxBytes << "\n";
            //out<<"  RxBytes:                     " << i->second.rxBytes << "\n";            
            //std::cout << "  TxPackets:                   " << i->second.txPackets << "\n";
            //out<<"  TxPackets:                   " << i->second.txPackets << "\n";            
            //std::cout << "  RxPackets:                   " << i->second.rxPackets << "\n";
            //out<<"  RxPackets:                   " << i->second.rxPackets << "\n";            
            //std::cout << "  LostPackets:	               " << (i->second.lostPackets) << "\n";
            //out<<"  LostPackets:	               " << (i->second.lostPackets) << "\n";            
            //std::cout << "  DelaySum:                    " << i->second.delaySum.GetSeconds()<< " s\n";
            //out<<"  DelaySum:                    " << i->second.delaySum.GetSeconds()<< " s\n";
            //std::cout << "  DelayMean:                   " << (i->second.delaySum.GetSeconds()) / (i->second.rxPackets) << " s\n";
            //out<<"  DelayMean:                   " << (i->second.delaySum.GetSeconds()) / (i->second.rxPackets) << " s\n";
            ...
            //out<<"  TimeLastTxPacket:            " << i->second.timeLastTxPacket.GetSeconds() << " s\n";
            //std::cout << "  TimeFirstRxPacket:           " << i->second.timeFirstRxPacket.GetSeconds() << " s\n";
            //out<<"  TimeFirstRxPacket:           " << i->second.timeFirstRxPacket.GetSeconds() << " s\n";
            //std::cout << "  TimeLastRxPacket:            " << i->second.timeLastRxPacket.GetSeconds() << " s\n";
            //out<<"  TimeLastRxPacket:            " << i->second.timeLastRxPacket.GetSeconds() << " s\n";
            //std::cout << "  MeanTransmittedPacketSize:   " << (i->second.txBytes) / (i->second.txPackets) << " byte\n";
            //out<<"  MeanTransmittedPacketSize:   " << (i->second.txBytes) / (i->second.txPackets) << " byte\n";
            ...
            //out<<"  MeanTransmittedBitrate:      " << ((i->second.txBytes) * 8.0) / ((i->second.timeLastTxPacket.GetSeconds())-(i->second.timeFirstTxPacket.GetSeconds()))  << " bit/s\n";
            //std::cout << "  MeanHopCount:                " << (i->second.timesForwarded) / (i->second.rxPackets) + 1 << "\n";
            //out<<"  MeanHopCount:                " << (i->second.timesForwarded) / (i->second.rxPackets) + 1 << "\n";
            //std::cout << "  PacketLossRatio:             " << (i->second.lostPackets) / ((i->second.rxPackets)+(i->second.lostPackets)) << "\n";
            //out<<"  PacketLossRatio:             " << (i->second.lostPackets) / ((i->second.rxPackets)+(i->second.lostPackets)) << "\n";            
            //std::cout << "  MeanReceivedPacketSize:      " << (i->second.rxBytes) / (i->second.rxPackets) << " byte\n";
            //out<<"  MeanReceivedPacketSize:      " << (i->second.rxBytes) / (i->second.rxPackets) << " byte\n";
            ...
            //out<<"  MeanReceivedBitrate:         " << ((i->second.rxBytes)* 8.0) / ((i->second.timeLastRxPacket.GetSeconds())-(i->second.timeFirstRxPacket.GetSeconds())) << " bit/s \n";
            //out<<"\n";
