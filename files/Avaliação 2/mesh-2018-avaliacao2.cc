...
MeshAulaRPG::Configure (int argc, char *argv[])
{
  ...
  cmd.AddValue ("jumps", "Nó onde se iniciará o envio de pacotes", m_jumps);
...
MeshAulaRPG::InstallApplication ()
{
  ...
  for (int index=1; index <= m_numFlows; ++index) {
      ...
      candidateSource = m_jumps - 1;
      //std::cout << "Candidate Source: " << candidateSource <<"\n";
  ...
  {        
        ...
        if (i->second.rxBytes > 0){
            ...
            if(m_packetInterval == 0.1){
                ...
                if(m_packetSize == 1024)
                {
                    out.open("avaliacao2/01/taxaEntrega.csv", std::ios::app);
                    out<<(i->second.rxPackets * 100.0)/ (i->second.txPackets)  << ";\n";
                    out.close();
                    
                    out.open("avaliacao2/01/vazao.csv", std::ios::app);
                    out<<i->second.rxBytes * 8.0 / 10.0 / 1024 / 1024  << ";\n";
                    out.close();
                    
                    out.open("avaliacao2/01/atrasoMedio.csv", std::ios::app);
                    out<<(i->second.delaySum.GetSeconds()) / (i->second.rxPackets) << ";\n";
                    out.close();
                    
                    out.open("avaliacao2/01/variacaoAtraso.csv", std::ios::app);
                ...
                if(m_packetSize == 1024)
                {
                    out.open("avaliacao2/001/taxaEntrega.csv", std::ios::app);
                    out<<(i->second.rxPackets * 100.0)/ (i->second.txPackets)  << ";\n";
                    out.close();
                    
                    out.open("avaliacao2/001/vazao.csv", std::ios::app);
                    out<<i->second.rxBytes * 8.0 / 10.0 / 1024 / 1024  << ";\n";
                    out.close();
                    
                    out.open("avaliacao2/001/atrasoMedio.csv", std::ios::app);
                    out<<(i->second.delaySum.GetSeconds()) / (i->second.rxPackets) << ";\n";
                    out.close();
                    
                    out.open("avaliacao2/001/variacaoAtraso.csv", std::ios::app);
                    out<<(i->second.jitterSum.GetSeconds()) / (i->second.rxPackets -1)<< ";\n";
                    out.close();
                ...
                if(m_packetSize == 1024)
                {
                    out.open("avaliacao2/0001/taxaEntrega.csv", std::ios::app);
                    out<<(i->second.rxPackets * 100.0)/ (i->second.txPackets)  << ";\n";
                    out.close();
                    
                    out.open("avaliacao2/0001/vazao.csv", std::ios::app);
                    out<<i->second.rxBytes * 8.0 / 10.0 / 1024 / 1024  << ";\n";
                    out.close();
                    
                    out.open("avaliacao2/0001/atrasoMedio.csv", std::ios::app);
                    out<<(i->second.delaySum.GetSeconds()) / (i->second.rxPackets) << ";\n";
                    out.close();
                    
                    out.open("avaliacao2/0001/variacaoAtraso.csv", std::ios::app);
                    out<<(i->second.jitterSum.GetSeconds()) / (i->second.rxPackets -1)<< ";\n";
                    out.close();
